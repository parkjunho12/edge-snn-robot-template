...
class TRTRuntime:
    ...
    def __init__(
        ...
        logger.info(
            f"TensorRT runtime initialized (v{self.trt_version}, API v{'2' if self.use_v2_api else '1'})"
        )
    ...
    def _allocate_buffers_v8(self):
        ...
        for i in range(self.num_bindings):
            ...
            logger.info(
                f"Binding {i}: {binding_name}, shape={shape}, dtype={dtype}, is_input={is_input}"
            )
            ...
            if -1 in shape:
                logger.info(
                    f"Dynamic shape detected for {binding_name}, will allocate on first inference"
                )
    ...
    def _allocate_buffers_v10(self):
        ...
        for i in range(self.engine.num_io_tensors):
            ...
            try:
                ...
                logger.info(
                    f"Tensor {i}: {tensor_name}, shape={shape}, dtype={dtype}, mode={mode}"
                )
                ...
                if -1 in shape:
                    logger.info(
                        f"Dynamic shape detected for {tensor_name}, will allocate on first inference"
                    )
                    if is_input:
                    ...
                    if is_input:
                        self.input_tensors[tensor_name] = {
                            "host": host_mem,
                            "device": device_mem,
                        }
                    else:
    ...
    def _allocate_dynamic_buffers(
        self, input_data: np.ndarray, input_name: str = "emg"
    ):
        ...
        for i in range(self.num_bindings):
            if not self.engine.binding_is_input(i) and self.outputs[i] is None:
                ...
                if -1 not in output_shape:
                    ...
                    logger.info(
                        f"Allocated output buffer for {output_name}: shape={output_shape}"
                    )
    ...
    def _infer_v10(self, input_data: np.ndarray, input_name: str = "emg") -> np.ndarray:
        ...
        # Allocate input buffer if needed
        if (
            input_name not in self.input_tensors
            or self.input_tensors[input_name] is None
        ):
            size = np.prod(input_shape)
        ...
        self.context.set_tensor_address(
            input_name, int(self.input_tensors[input_name]["device"])
        )
        ...
        for output_name in self.output_names:
            ...
            if (
                output_name not in self.output_tensors
                or self.output_tensors[output_name] is None
            ):
                if -1 not in output_shape:
                    size = np.prod(output_shape)
                    dtype = self.output_dtypes[output_name]
                    host_mem = cuda.pagelocked_empty(int(size), dtype)
                    device_mem = cuda.mem_alloc(host_mem.nbytes)
                    self.output_tensors[output_name] = {
                        "host": host_mem,
                        "device": device_mem,
                    }
                    logger.info(
            ...
            if (
                output_name in self.output_tensors
                and self.output_tensors[output_name] is not None
            ):
                self.context.set_tensor_address(
                    output_name, int(self.output_tensors[output_name]["device"])
                )
    ...
    def infer_batch(
        self, input_batch: np.ndarray, input_name: str = "emg"
    ) -> np.ndarray:
        """
