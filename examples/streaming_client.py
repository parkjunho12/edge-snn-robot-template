...
class StreamingInferenceClient:
    ...
    def single_inference_pytorch(
        self, batch: int = 1, channels: int = 16, length: int = 200, steps: int = 1
    ) -> dict:
    ...
    def single_inference_tensorrt(
        self, batch: int = 1, channels: int = 16, length: int = 200
    ) -> dict:
        """Run single inference with TensorRT"""
        response = requests.post(
            f"{self.base_url}/infer/tensorrt",
            json={"batch": batch, "channels": channels, "length": length, "steps": 1},
        )
        response.raise_for_status()
        return response.json()
    ...
    def benchmark(self, num_iterations: int = 1000) -> dict:
        """Run benchmark"""
        response = requests.post(
            f"{self.base_url}/benchmark", params={"num_iterations": num_iterations}
        )
        response.raise_for_status()
        return response.json()
...
def example_streaming_inference():
    ...
    client.stream_inference(
        duration_seconds=10.0, use_tensorrt=False, fps=30, preprocess=True
    )
...
def example_streaming_with_callback():
    ...
    client = StreamingInferenceClient()
    client.stream_inference(
        duration_seconds=5.0, use_tensorrt=True, fps=30, callback=custom_callback
    )
...
def main():
    ...
    parser.add_argument(
        "--example", type=int, choices=range(1, 7), help="Run specific example (1-6)"
    )
    parser.add_argument("--stream", action="store_true", help="Run streaming inference")
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-tensorrt", action="store_true", help="Disable TensorRT backend"
    )
    ...
    if args.stream:
        ...
        client.stream_inference(
            duration_seconds=args.duration, use_tensorrt=False, fps=args.fps
        )
    elif args.example:
